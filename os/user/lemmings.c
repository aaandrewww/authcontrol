...
static uint8_t heapBuffer[4*LEMMINGS*PGSIZE];
...
umain(int argc, char **argv){
  Heap heap;
  init_heap(&heap, &heapBuffer, 4*LEMMINGS*PGSIZE);
  set_heap(&heap);
  int i;
  envid_t who;
  Proof deligations[LEMMINGS];
  envid_t whos[LEMMINGS];

  for(i = 0; i < LEMMINGS; i++){
    who = fork();
    if(who == 0){
      //child
  ...
  if(who != 0) {
    ...
    Proof parent_ok = says_from_signed(thisenv->env_id, pred);
    send_proof(who, parent_ok);
  } else {
    // child
