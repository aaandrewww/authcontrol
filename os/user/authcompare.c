...
void umain(int argc, char **argv) {
  ...
  if (who == 0) {
    ...
    if (whos[0] != 0) {
      ...
      x = read_tsc() - x;
      cprintf("time %u, avg %u\n", x, x/PAGESTOMAP);

      ipc_send(other_child, 55, 0, 0);

      // IPC method
//      cprintf("in env %u, IPC mapping to %u\n", thisenv->env_id, other_child);
//      uint32_t x = read_tsc();
//      for (i = 0; i < PAGESTOMAP; i++) {
//        vaddr = (uintptr_t)UTEMP + i*PGSIZE;
//        if (sys_page_alloc(0, UTEMP, PTE_U | PTE_W | PTE_P) < 0)
...
//          test[i] = i;
//
//        ipc_send(other_child, 0, UTEMP, PTE_U | PTE_W | PTE_P);
//      }
//      x = read_tsc() - x;
//      cprintf("time %u, avg %u\n", x, x/PAGESTOMAP);
    ...
    if (whos[0] == 0) {
      ...
      // IPC Method
//      uintptr_t utempVal = (uintptr_t)UTEMP;
//      for( i = 0; i < PAGESTOMAP; i++){
//        do {
//          r = ipc_recv(0, (void*) (utempVal + i*PGSIZE), 0);
//        } while (r == -E_AGAIN);
//      }
