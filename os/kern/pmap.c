...
boot_alloc(uint32_t n)
{
	...
	// If n==0, return the address of the next free page
	if (n == 0) return (void *) nextfree;

	// n > 0: Need to allocate pages
	uint32_t to_allocate = ROUNDUP((uint32_t)n, PGSIZE) / PGSIZE;
	//cprintf("boot_alloc:\n nextfree: %x\n page_num: %d\n to_allocate: %d\n", nextfree, (uint32_t) nextfree / PGSIZE, to_allocate);

	// See if we have enough memory left
	if ((((size_t) nextfree - KERNBASE) / PGSIZE) + to_allocate > npages)
		panic("boot_alloc: out of memory\n");
	// Good! allocate some memory and increment nextfree
	result = nextfree;
	nextfree = nextfree + (to_allocate * PGSIZE);
	
	//cprintf("New nextfree: %x page_num: %d\n", nextfree, (uint32_t) nextfree / PGSIZE);
	return result;
}
...
free_page_count(void)
{
	...
	for (Page *pp = page_free_list; pp; pp = pp->pp_link) {
		free_pages++;
	}
	return free_pages;
}
...
page_alloc(void)
{
	...
	//uint32_t free = free_page_count();

	Page *allocated = page_free_list;
	// Take the allocated page off of the free list
	page_free_list = allocated->pp_link;
	allocated->pp_link = 0;
 
	//assert(free_page_count() == free - 1);
	return allocated;
}
...
page_free(struct Page *pp)
{
	...
	if (pp->pp_ref != 0) panic("Attempting to free page with ref count != 0");
	
	uint32_t free = free_page_count();
	// Add the page to the front of the free list
...
void
print_page_table(pte_t *table,  bool pdt, bool tab) {
	if (tab) cprintf("\t");
	if (pdt) cprintf("\n");
	cprintf("Offset      Page address  U/S  R/W  P\n");
	if (tab) cprintf("\t");
	cprintf("-------------------------------------\n");
	for (uint32_t i=0; i<NPTENTRIES; i++) {
		...
		if (entry & PTE_P) {
			...
			cprintf("0x%08x  0x%08x     %x    %x   %x\n",
				offset,
				PTE_ADDR(entry),
				(entry & PTE_U) >> 2,
				(entry & PTE_W) >> 1,
				(entry & PTE_P));
			if (pdt) print_page_table((pte_t *)KADDR(PTE_ADDR(entry)), false, true);
		}
	}
	if (tab) cprintf("\t");
	cprintf("=====================================\n");
...
{
	// la and pa should be page aligned
	assert(PGALIGNED(la));
	assert(PGALIGNED(pa));
	uintptr_t cur_la = la;
	physaddr_t cur_pa = pa;
	for (uint32_t i=0; i<size/PGSIZE; i++) {
		// Locate the page table for this logical address
		pte_t *pte = pgdir_walk(pgdir, cur_la, true);
		if (pte == NULL) panic("page_map_segment: Failed to allocate page table!\n");
		
		// Install a new entry
		// Use address of physical address (page aligned) OR'ed with perms
...
{
	// LAB 3: Your code here.
	// Check that the range is within [0, ULIM)
	if ((va + len < va) || (va + len >= ULIM)) {
		user_mem_check_addr = (va < ULIM) ? ULIM : va;
		return -E_FAULT;
	}

	// Check permissions on the appropriate pages
	uintptr_t page = va & 0xFFFFF000; // Round down to nearest page
	uintptr_t limit = va+len;
	for (; page < limit; page += PGSIZE) {
		pte_t *pte = pgdir_walk(env->env_pgdir, page, false);
		if (!pte || !CHECKPERM(*pte,(perm | PTE_P))) {
			user_mem_check_addr = (page < va) ? va : page;
			return -E_FAULT;
		}
	}
