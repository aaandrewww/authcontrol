...
handle_flush_queue()
{
	...
	if ((r = fstat(p[0], &qstats)) < 0)
		panic("Error handling flush queue\n");
	
	while (qstats.st_size >= (ssize_t) sizeof(int32_t)) {
		ssize_t sofar = 0;
		while (sofar < (ssize_t) sizeof(int32_t)) {
			r = read(p[0], &breq, sizeof(int32_t));
			if (r < 0) panic("Error handling flush queue\n");
			sofar += r;
		}
		blocknum_t blocknum = BCREQ_BLOCKNUM(breq);
		flush_block(blocknum);
